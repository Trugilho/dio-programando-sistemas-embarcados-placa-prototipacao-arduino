...
// Definindo os simbolos do Teclado
    ...
    DigitarCadeia: // Marcação para voltar depois
      ...
      while (true)
      {
        ...
        if (key)
        {
          ...
          {
            lcd.clear();
            for (contador = 1; contador <= cadeia.length(); contador++)
            { // Percorre a Cadeia digitada
              lcd.clear();
              lcd.setCursor(0, 0);
              lcd.print(">");
              lcd.print(cadeia);
            ...
            for (int i = 0; i < 3; i++)
            {
              ...
              delay(300);
              lcd.setCursor(0, 0);
              if (estados_finais.indexOf(estado_atual) != -1)
...
{
  for (int i = 0; i < qtd_estados.toInt() * 3; i++)
  { // Percorre as transições até encontrar a certa
